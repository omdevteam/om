...
Utilities for the processing of Small And Wide Angle X-Ray Scattering data (radial
pixel map calculation, radial profile scaling, etc.).
"""
from __future__ import absolute_import, division, print_function

import numpy
import from scipy import constants, stats

from onda.utils import named_tuples


def pixel_bins_to_q_bins(detector_distance,beam_energy,pixel_size,pixel_radial_bins,coffset,radial_bin_size):
    """
    Converts pixel-space radial bins to q-space radial_bins.

    Given a fixed number of radial bins, and a fixed size of each bin, computes the
    bin edges for specific detector distance and energy values.
    ...
    Args:
        ...
        coffset (float) : detector distance's offset in meters (adjustment to the
            detector distance reported by the facility).

        radial_bin_size (float): size of each bin in pixels

    Returns:

        ndarray: array of q values associated with each radius bin in inverse
        angstroms.
    """
    # TODO: Check the calculation. The units are now SI.
    lambda_ = (scipy.constants.h * scipy.constants.c) / beam_energy
    theta = 0.5 * numpy.arctan((pixel_radial_bins * radial_bin_size * pixel_size) / (detector_distance + coffset))
...
def calculate_radial_bin_info(radius_pixel_map, num_bins):
    ...
    Calculates a pixel map containing radial bin information for each pixel.

       Args:

           pixelmap_radius (ndarray): radial pixel map.

           num_radial_bins (int): number of radial bins required by the user.
    ...
    for i in range(0, num_bins - 1):
        radial_bin_pixel_map[(radius_pixel_map >= i * deltar) & (radius_pixel_map < (i + 1) * deltar)] = i
        radial_bin_pixel_map[radius_pixel_map >= num_bins * deltar] = num_bins

    return named_tuples.RadialBinInfo(radial_bin_pixel_map=radial_bin_pixel_map, radial_bin_size=deltar)
...
def scale_profile(radial_profile, min_radial_bin, max_radial_bin):
    ...
    The scaling is based on the average intensity value in the radial bin region
...
def calculate_avg_radial_intensity(data, radial_bin_pixel_map, mask):
    ...
    The input data is split into radial bins according to the provided radius bin
    pixel map. An average intensity is then computed for each bin.

    Args:

        data (ndarray): frame data.

        radial_bin_pixel_map (ndarray): pixel map describing the radius bin each
            pixels falls into.
    ...
    radial_average = scipy.ndimage.sum(data, labels=radial_bin_pixel_map, index=idx)
    # mask = numpy.ones(data.shape)
    # mask[data==0] = 0
    mask_radial = scipy.ndimage.sum(mask, labels=radial_bin_pixel_map, index=idx)
...
def mask_panel(panel,sub_sh=(32, 32),thresh=7):
    ...
    masks = numpy.zeros(subs.shape, numpy.bool)
