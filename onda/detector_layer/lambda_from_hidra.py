...
def raw_data(event):
    return event.filehandle['/entry/instrument/detector/data'][event.shot_offset, :, :]
...
def detector_distance(event):
    return event.monitor_params['General']['fallback_detector_distance']


def beam_energy(evt):
    return evt.monitor_params['General']['fallback_beam_energy']
