...
Functions and classes used to retrieve data from the Pilatus detector as used at the
Petra III facility.
"""
...
def get_peakfinder8_info():
    ...
    Retrieves the peakfinder8 information matching the data format used by the Pilatus
...
def open_event(event):
    ...
    Makes the content of a Pilatus event retrieved at the petra III facility available
    ...
    # object in the 'data' entry of the event dictionary.
...
def get_num_frames_in_event(event):
    ...
    """
    del event
    # Each event from a Pilatus detector usually contains only a single frame.
    return 1
...
def detector_data(event):
    ...
    Extracts one frame of Pilatus detector data from an event retrieved at the Petra
    III facility.
    ...
    """
    # Extracts frame information from the data stored in the event dictionary.
...
def event_id(event):
    ...
    experiment. When using the Pilatus detector at the Petra III facility, the full
    path of the file where the current event has been saved is used as an identifier.
...
def frame_id(event):
    ...
    within the event. When using the Pilatus detector at the Petra III facility, the
    index of the current frame within the file where the event has been saved is used
    as an identifier.
