...
class OndaMonitor(mpi.ParallelizationEngine):
    ...
    Broadcast reduced data for visualization. Optionally also broadcast corrected
    ...
    def __init__(self, source, monitor_parameters):
        ...
        Args:
            ...
            source (str): A string describing the data source. The exact format of the
                string depends on the data recovery layer used by the monitor (e.g:the
                string could be a psana experiment descriptor at the LCLS facility,
                the IP of the machine where HiDRA is running at the PetraIII facility,
                etc.)

            monitor_params (MonitorParams): a
                :obj:`~onda.utils.parameters.MonitorParams` object containing the
                monitor parameters from the configuration file.
        """
        ...
        num_radial_bins = monitor_parameters.get_param(
            section="Radial", parameter="num_radial_bins", type_=int, required=True
        )
        ...
        # Creates a pixel map in which each data pixel is labelled according to the
        # radial bins it belongs to. Used for radial averagning. Also computes the
        # readial bin size.
        radial_bin_info = swaxs_utils.calculate_radial_bin_info(
            radius_pixel_map=radius_pixel_map, num_bins=num_radial_bins
        )
        self._radial_bin_pixel_map = radial_bin_info.radial_bin_pixel_map
        self._radial_bin_size = radial_bin_info.radial_bin_size

        # Redefines the number of radial bins to the number of radial bins for which at
        # least a pixel is present. Bins that will never be filled are removed.
        ...
        self._scale = monitor_parameters.get_param(
            section="Radial", parameter="scale", type_=bool, required=True
        )
        self._scale_region_begin = monitor_parameters.get_param(
            section="Radial", parameter="scale_region_begin", type_=int, required=True
        )
        self._scale_region_end = monitor_parameters.get_param(
            section="Radial", parameter="scale_region_end", type_=int, required=True
        )

        # Reads from the configuration file the total intensity threshold for a
        # detector frame to be considered a hit.
        ...
        # Reads from the configuration file the information used to draw the intensity
        # sum histogram.
        ...
        self._intensity_sum_hist_bins = numpy.linspace(
            intensity_sum_hist_min, intensity_sum_hist_max, intensity_sum_hist_num_bins
        )

        # Creates an empty intensity sum histogram.
        self._intensity_sum_hist, self._intensity_sum_hist_bins = numpy.histogram(
            [0], self._intensity_sum_hist_bins
        )
        ...
        if self.role == "worker":
            ...
            requested_grouping_alg = monitor_parameters.get_param(
                section="Grouping", parameter="grouping_algorithm", type_=str
            )

            if requested_grouping_alg is not None:
                grouping_alg_class = getattr(group_algs, requested_grouping_alg)

                self._grouping_alg = grouping_alg_class(monitor_parameters)
            else:
                # If no calibration is required, stores None in the 'calibration_alg'
                # attribute.
            ...
            # instantiate the dark calibration correction algorithm, then instatiate
            # the algorithm.
            ...
            # TODO: Make mask available
            self._file_mask = self._dark_cal_corr_alg._mask
            self._pixel_mask_threshold = monitor_parameters.get_param(
                section="Radial", parameter="pixel_threshold_in_ADU", type_=int
            )

            # Reads from the configuration file the information for profile subtraction.
            profile_for_subtr_fname = monitor_parameters.get_param(
                section="Radial", parameter="subtract_profile_filename", type_=str
            )
            if profile_for_subtr_fname:
                self._profile_for_subtraction = numpy.loadtxt(
                    profile_for_subtr_fname, usecols=(0, 1)
                )
            else:
        ...
        if self.role == "master":
            # Reads from the configuration file whether the experiment pump-probe or
            # not.
            ...
            self._recent_profiles = numpy.zeros((num_profiles, num_radial_bins))
            self._recent_pumped_profiles = numpy.zeros((num_profiles, num_radial_bins))
            self._recent_dark_profiles = numpy.zeros((num_profiles, num_radial_bins))
            ...
            # Reads from the configuration file how many events should be accumulated
            # by the master node before broadcasting the accumulated data. Then
            # instantiates the data accumulator.
            ...
            self._cumulative_unscaled_radial_mean = numpy.zeros(num_radial_bins)
            self._cumulative_unscaled_radial_std = numpy.zeros(num_radial_bins)
            ...
            self._cumulative_unscaled_stack = numpy.zeros((num_groups, num_radial_bins))
            self._cumulative_scaled_stack = numpy.zeros((num_groups, num_radial_bins))
            self._cumulative_diff_stack = numpy.zeros((num_groups, num_radial_bins))
    ...
    def process_data(self, data):
        ...
        Performs detector and dark calibration corrections, if required, computes the
        radial average and converts the radial average to q coordinates.
        ...
        corr_det_data = numpy.nan_to_num(
            self._dark_cal_corr_alg.apply_darkcal_correction(data=calib_det_data)
        )
        ...
        pixel_size = 1.0 / list(self.geometry["panels"].items())[0][1]["res"]
    ...
    def collect_data(self, data):
        ...
        the aggregated data (e.g.: averages and cumulative averages, etc.) Finally,
        broadcasts the reduced data for visualization.
        ...
        self._intensity_sum_hist += numpy.histogram(
            results_dict["intensity_sum"], self._intensity_sum_hist_bins
        )[0]
        ...
        # Sums up the unscaled_radials to make the cumulative average rather than
        # scaling first and then averaging. This should help to mitigate noise from
        # weak profiles.
        ...
        # Runs a second hit finder. This second hit finder filters out frames whose
        # radial profile in the scaling region is more than 3 standard deviations
        # outside of the mean. It does
        ...
        self._cumulative_unscaled_radial_std = numpy.sqrt(
            (self._cumulative_unscaled_radial_sq / self._num_events)
        ...
        std_dev_hit = numpy.all(
            radial_subset < numpy.abs(radial_subset - (mean_subset + 30 * std_subset))
        )

        if self._num_events < self._cumulative_diff_stack.shape[0]:
            std_dev_hit = True

        hit_flag = numpy.logical_and(results_dict["cand_hit_flag"], std_dev_hit)

        self._hit_rate_run_wdw.append(float(hit_flag))

        # Divides by the window size to compute the average hit rate, but only if the
        # window has already been filled with events. Otherwise takes the number of
        # events that the window alreadycontains.
        avg_hit_rate = sum(self._hit_rate_run_wdw) / min(
            self._run_avg_wdw_size, self._num_events
        )
        ...
        if results_dict["xrays_active"]:
            ...
            if results_dict["optical_laser_active"]:
                ...
                # Divides by the window size to compute the average hit rate, but only
                # if the window has already been filled with events. Otherwise takes
                # the number of events that the window already contains.
                pumped_avg_hit_rate = sum(self._pumped_hit_rate_run_wdw) / min(
                    self._run_avg_wdw_size, self._num_pumped
                )
                results_dict["pumped_hit_rate"] = pumped_avg_hit_rate

                # The GUI cannot read the configuration file, so it does not know if
                # the experiment is pump probe or not. A None value sent to the GUI if
                # the experiment is not pump probe.
                ...
                if hit_flag:
                    ...
                    self._cumulative_dark_avg = self._cumulative_dark / self._num_dark

                    # Calculates the running average of recently collected profiles
                    # only.
                    self._recent_dark_profiles = numpy.roll(
                        self._recent_dark_profiles, -1, axis=0
                    )
                    self._recent_dark_profiles[-1] = unscaled_radial
                    self._recent_dark_avg = numpy.mean(
                        self._recent_dark_profiles, axis=0
                    )
                ...
                # if the window has already been filled with events. Otherwise takes
                # the number of events that the window already contains.
                dark_avg_hit_rate = sum(self._dark_hit_rate_run_wdw) / min(
                    self._run_avg_wdw_size, self._num_dark
                )
        ...
        self._recent_radial_avg = self._recent_pumped_avg - self._recent_dark_avg
        ...
        # Any frame_id greater than expected in the list of frames is piled into the
        # last frame expected.
        group_id = min(
            int(results_dict["group_id"]), self._cumulative_unscaled_stack.shape[0]
        )

        if (
            results_dict["xrays_active"]
            and results_dict["optical_laser_active"]
            and hit_flag
        ):
            self._cumulative_unscaled_stack[group_id] += unscaled_radial

            if self._scale:
                self._cumulative_scaled_stack[group_id] = swaxs_utils.scale_profile(
                    self._cumulative_unscaled_stack[group_id],
                    self._scale_region_begin,
                    self._scale_region_end,
                )
            else:
                self._cumulative_scaled_stack[
                    group_id
                ] = self._cumulative_unscaled_stack[group_id]

            self._cumulative_diff_stack[group_id] = (
                self._cumulative_scaled_stack[group_id] - self._cumulative_dark_avg
            )
