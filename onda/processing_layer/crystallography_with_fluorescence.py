...
class OndaMonitor(mpi.ParallelizationEngine):
    ...
    def __init__(self,
        ...
        if self.role == 'worker':
            ...
            self._threshold_second_detector = monitor_parameters.get_param(
                section='General',
        ...
        if self.role == 'master':
            ...
            self._num_events_to_accumulate = pa_num_events_to_accumulate
    ...
    def process_data(self, data):
        ...
        sum_second_detector = (
            data['detector2_data'] > self._threshold_second_detector
        ).sum()
        ...
        results_dict['sum_first_detector'] = sum_first_detector
        results_dict['sum_second_detector'] = sum_second_detector
    ...
    def collect_data(self, data):
        ...
        if 'sum_second_detector' in results_dict:
            ...
            self._list_event_sum_det_two.append(
                results_dict['sum_second_detector']
            )
            if (
                    len(self._list_event_sum_det_one) ==
                    self._num_events_to_accumulate
            ):
                collected_two_det_data['sum_first_detector'] = (
                    self._list_event_sum_det_one
                )
                collected_two_det_data['sum_second_detector'] = (
                    self._list_event_sum_det_two
                )
