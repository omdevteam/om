...
Exports:
    ...
    Classes:
        ...
        OndaMonitor: an OnDA monitor for x-ray crystallography experiments.
"""
...
class OndaMonitor(mpi.ParallelizationEngine):
    ...
    def __init__(self, source, monitor_parameters):
        ...
        Args:
            ...
            source (str): A string describing the data source. The format
                of the string depends on the Data Recovery layer that the
                monitor uses (e.g: the string represents a psana experiment
                descriptor for the psana backend, the IP address where HiDRA
                is running for the Petra III backend, etc.)
        ...
        if self.role == 'worker':
            ...
            # algorithm should be used. Instantiate the calibration algorithm
            # and store it in an attribute, so it can be called later.
            # If no calibration has been store None in the same attribute.
            ...
            # then instantiate the algorithm and store it in an attribute so
        ...
        if self.role == 'master':
            ...
            #
            # num_events: to keep track of the number of processed events.
            #
            ...
            # should be, then initalize the windows and the averages.
            ...
            # used for the socket that broadcasts data to the GUI(s), then
    ...
    def collect_data(self, data):
        ...
        # and mark the event as collected, increasing the counter.
        results_dict, _ = data
        self._num_events += 1

        # Update the windows used to compute the running average, then compute
        # the new average hit and saturation rates.
        ...
        # If the peak accumulator algorithm returned some data (not None),
        # the accumulator has been reset: add the returned data to
        # the 'collected_data' dictionary and send it to the GUI.
        ...
        # node, it must be sent to the GUI: add it to the 'collected_rawdata'
        # dictionary.
        ...
        # speed should be reported to the user, use the time elapsed since
        # the last message  to compute the speed, and report everything to
        # the user.
