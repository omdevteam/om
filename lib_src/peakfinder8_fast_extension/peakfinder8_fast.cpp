...
static void compute_num_radial_bins(int num_pix, int *radius, int *num_rad_bins)
{
	int i;
	*num_rad_bins = 0;
	for (i = 0; i < num_pix; i++)
	{
		if (radius[i] + 1 > *num_rad_bins)
		{
			*num_rad_bins = radius[i] + 1;
		}
	}
}
												...
												float acd_threshold)
{
	...
	compute_num_radial_bins(num_pix, radius, &num_rad_bins);
						...
						float *sum_com_ss, float *sum_i, int max_pix_count)
{
	...
	for (k = 0; k < search_n; k++)
	{
		...
		// Above threshold?
		if (copy[pi] > curr_threshold && pfinter->pix_in_peak_map[pi] == 0 && mask[pi] != 0)
		{
						  ...
						  int local_bg_radius, float min_snr, int max_n_peaks)
{
	...
	for (pxss = 1; pxss < asic_size_ss - 1; pxss++)
	{
		for (pxfs = 1; pxfs < asic_size_fs - 1; pxfs++)
		{
			...
			if (copy[pxidx] > curr_thresh && pfinter->pix_in_peak_map[pxidx] == 0 && mask[pxidx] != 0)
			{ //??? not sure if needed
				...
				if (num_pix_in_peak < min_pix_count || num_pix_in_peak > max_pix_count)
					continue;
				...
				// different!
				if (peak_max_i < background_max_i - local_offset)
					continue;

				if (peak_com_fs < aifs * asic_size_fs || peak_com_fs > (aifs + 1) * asic_size_fs - 1 || peak_com_ss < aiss * asic_size_ss || peak_com_ss > (aiss + 1) * asic_size_ss - 1)
				{
					continue;
				}
							...
							char *outliersMask)
{
	...
	for (aiss = 0; aiss < num_asics_ss; aiss++)
	{
		...
		{														  // ??? to change to proper panels need
...
int peakfinder8(tPeakList *peaklist, float *data, char *mask, float *pix_r,
				int rstats_num_pix, int *rstats_pidx, int *rstats_radius,
