...
cdef extern from "peakfinder8_fast.hh":
...
def peakfinder_8(int max_num_peaks, float[:,::1] data, char[:,::1] mask,
                 ...
                 int[:] rstats_radius, long asic_nx, long asic_ny, long nasics_x,
    ...
    """
    peakfinder_8(max_num_peaks, data, mask, pix_r, rstats_num_pix, rstats_pidx, \
        rstats_radius, asic_nx, asic_ny, nasics_x, \
...
List[float], List[float]`: A tuple storing  information about the detected peaks. The
    ...
    peakfinder8(&peak_list, &data[0, 0], &mask[0, 0], &pix_r[0, 0], rstats_num_pix, 
                &rstats_pidx[0], &rstats_radius[0], asic_nx, asic_ny,
