...
def read_mll_logfile(fnam, verbose=True):
    ...
    if version == 0.1:
        print
        'old version of log file. Rerouting...'
        Log = Log_file_v01(fnam)
    elif version == 0.2:
        Log = Log_file_v02(fnam, verbose)
    elif version == 0.3:
        Log = Log_file_v03(fnam, verbose)
    elif version != version:
        print
        "unknown logfile version. Here's hoping..."
        Log = Log_file_v02(fnam, verbose)
    return Log
...
class Log_file_v03():
    ...
    version = 0.3
    header_tag = '#'
    slower_axis_header = 'Slower axis'
    ...
    def __init__(self, fnam, verbose=True):
        ...
        self.max_steps = self.log['Files']['steps'][-1, -1] + 1
    ...
    def read_mll_log(self, fnam=None):
        ...
        with open(fnam) as f:
            ...
            while True:
                ...
                if key == self.slow_axis_header or key == self.fast_axis_header or key == self.slower_axis_header:
            ...
            if data != []:
                ...
                # New problem with stay still hack!
                # Now there may be duplicate columns for the motor coords
                # The coordinates in the slow axis are static while the
                # coordinates for the 'hack' axis (always the last axis)
                # are the good ones.

                for axis in [self.fast_axis_header, self.slow_axis_header, self.slower_axis_header]:
                    if log.has_key(axis):
                        if log_out.has_key(log[axis]['name']):
                            log[axis].update(log_out[log[axis]['name']])
                            # del log_out[log[axis]['name']] # why do I delete?
        ...
        # -----------------------------------------
        fnams, steps, grid, physical_grid_axes = self.generate_nxs_filenames(log, include_dir=False)
        ...
        log['Grid'] = grid
        log['Physical_grid_axes'] = physical_grid_axes
        return log

    def generate_nxs_filenames(self, log, include_dir=True):
        """
        """
        # figure out which axis are scaned over
        # slow --> fast
        axes = []
        grid = []
        physical_grid_axes = []
        for axis_header in [self.slower_axis_header, self.slow_axis_header, self.fast_axis_header]:
            if axis_header in log.keys():
                axes.append(log[axis_header])

                # record the grid
                if 'Repeat' in axes[-1].keys() and axes[-1]['Repeat'] != 0:
                    grid.append(axes[-1]['Repeat'] + 1)

                # if it is a physical axis, rather than a repeat or hack, then record it
                if 'StayStill hack' not in axes[-1].keys() or axes[-1]['StayStill hack'] is not True:
                    physical_grid_axes.append(len(grid))

                grid.append(axes[-1]['Steps'])

        # calculate the predicted total number of points
        if len(grid) == 0:
            no_steps = 0
        else:
            no_steps = reduce(lambda x, y: x * y, grid)

        # calculate the step numbers on the grid
        steps = np.arange(no_steps)

        # if the last axis is a fly scan then all of those steps
        # are in a single file
        if axes[-1]['Scan type'] == 'Fly':
            steps = steps.reshape((-1, grid[-1]))
        # otherwise each file is a single frame
        else:
            steps = steps[..., np.newaxis]

        fnams = []
        for i in range(steps.shape[0]):
            fnam = make_nxs_filename(log, steps[i][0], include_dir=False)
            fnams.append(fnam)
        return fnams, steps, grid, physical_grid_axes
...
class Log_file_v01(Log_file_v02):
    ...
    def get_axis_att(self, file, key, value, stop=None):
        ...
        # Random position [%]: 10.00
        """
        attrs = ['Current position', 'Start position', 'End position', 'Step size', 'Steps', 'Random position',
                 'Scan type']
        axis = {}
        axis['name'] = value
        axis['Scan type'] = 'Step'
...
def update_progress(progress, algorithm, i, fnam):
    ...
    text = "\r{0}: [{1}] {2}% {3} {4} {5} {6}".format(algorithm, "#" * block + "-" * (barLength - block),
                                                      int(progress * 100), i, fnam, status,
                                                      " " * 5)  # this last bit clears the line
...
def parse_motor_log(fnam_motor_log, redo=False, verbose=False):
            ...
            for no, line in enumerate(f):
                ...
                if line[0].isdigit():
                    ...
                    for i, p in enumerate(pos):
                        ...
                        if verbose: print
                        i, p, col_names[i], att_names[i], convert(p), motor_pos[col_names[i]][att_names[i]]['unit']

                        if p == '{}' or p == 'None':
                            motor_pos[col_names[i]][att_names[i]]['value'].append(np.nan)
                        else:
                            motor_pos[col_names[i]][att_names[i]]['value'].append(
                                convert(p) * motor_pos[col_names[i]][att_names[i]]['unit'])
    ...
    for key in motor_pos.keys():
        for att in motor_pos[key].keys():
            print
            key, att, len(motor_pos[key][att]['value'])
