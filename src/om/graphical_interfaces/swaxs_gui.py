...
class SwaxsGui(OmGuiBase):
    ...
    def __init__(self, url: str) -> None:
        ...
        #radial profiles
        self._radial_widget = pyqtgraph.PlotWidget()
        ...
        self._radial_plot: Any = self._radial_widget.plot(
            tuple(range(1000, 0)), [0.0] * 1000,
            name="frame"
        )
        self._recent_avg_plot: Any = self._radial_widget.plot(
            tuple(range(1000, 0)), [0.0] * 1000,
            pen=pyqtgraph.mkPen("y"),
            name="recent"
        )
        # self._cumulative_avg_plot: Any = self._radial_widget.plot(
        #     tuple(range(1000, 0)), [0.0] * 1000,
        #     pen=pyqtgraph.mkPen("c"),
        #     name="cumulative"
        # )

        self._roi_widget = pyqtgraph.PlotWidget()
        ...
        self._int_widget = pyqtgraph.PlotWidget()
        ...
        self._int_widget.setLabel(axis="left", text="Ratio of Transmitted/Incident Intensity")
        ...
        # self._rg_plot: Any = self._int_widget.plot(
        #     pen=None, 
        #     symbol='o',
        #     symbolPen=pyqtgraph.mkPen("c"),
        #     symbolSize=3,
        ...
        #svd 
        # self._svd_widget = pyqtgraph.PlotWidget()
        # self._svd_widget.setLabel(axis='bottom', text='q (1/angstrom)')
        # self._svd_plots = []
        # colors = [(255,255,255),(0,255,255),(0,255,0),(255,255,0),(255,0,0)]
        # for i in range(5): 
        #     self._svd_plots.append(
        #         self._svd_widget.plot(
        #             tuple(range(1000, 0)), [0.0] * 1000,
        #             name="svd_%d"%i,
        #             pen=pyqtgraph.mkPen(colors[i])
        #         )
        #     )
        ...
        cmap = pyqtgraph.ColorMap(pos,colors)
        self._radial_stack_view.setColorMap(cmap)
        #self._radial_stack_view.addItem(self._radial_stack)
        ...
        #splitter_0.addWidget(self._radial_widget)
        #splitter_1: Any = QtWidgets.QSplitter()
        #splitter_1.addWidget(self._radial_widget)
        #splitter_1.addWidget(self._svd_widget)
        ...
        #vertical_splitter.addWidget(self._svd_widget)
    ...
    def update_gui(self) -> None:
        ...
        radial = local_data["radial"]
        q = local_data["q"]

        self._radial_plot.setData(
            q, radial 
        )
        recent_avg = local_data["recent_radial_average"]
        self._recent_avg_plot.setData(
            q, recent_avg 
        )
        # cumulative_avg = local_data["cumulative_radial_average"]
        # self._cumulative_avg_plot.setData(
        #     q, cumulative_avg 
        # )
        ...
        self._roi1_plot.setData(
            tuple(range(-5000, 0)), local_data["roi1_int_history"]
        )
        self._roi2_plot.setData(
            tuple(range(-5000, 0)), local_data["roi2_int_history"]
        )

        with numpy.errstate(divide='ignore', invalid='ignore'):
            # ratio = numpy.nan_to_num(local_data["downstream_monitor_history"]/local_data["upstream_monitor_history"])
            ratio = numpy.nan_to_num(local_data["upstream_monitor_history"])

        self._int_monitor_ratio_plot.setData(
            tuple(range(-5000, 0)), ratio
        )

        self._digitizer_sum_plot.setData(
            tuple(range(-5000, 0)), local_data["digitizer_sum_history"]
        )

        # self._rg_plot.setData(
        #  tuple(range(-5000, 0)), numpy.nan_to_num(local_data["upstream_monitor_history"])
        # )

        # self._rg_plot.setData(
        #     tuple(range(-5000, 0)), local_data["rg"]
        # )

        # svd = local_data["svd_vectors"]
        # rbins_svd = numpy.arange(len(svd[0]))
        # theta_svd =  numpy.arctan( pixsize*rbins_svd / detdist ) *.5
        # q_svd = numpy.sin( theta_svd ) * 4 * numpy.pi / wavelen
        # for i in range(svd.shape[0]):
        #     if svd[i].sum() < 0:
        #         svd[i] *= -1
        #     self._svd_plots[i].setData(
        #         q_svd, svd[i] + 0.1*i
        #     )
