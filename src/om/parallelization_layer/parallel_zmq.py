...
from om.library import exceptions, parameters, zmq_collecting
...
def _om_processing_node(
    *,
    ...
    sender_push.connect(f"tcp://{zmq_collecting.get_current_machine_ip()}:5555")

    # This is where the worker servers recieve instruction (Sub)
    # from collecting layer (Pub)

    socket_sub = context.socket(zmq.SUB)
    socket_sub.connect(f"tcp://{zmq_collecting.get_current_machine_ip()}:5556")
    ...
    socket_sub.setsockopt_string(option=zmq.SUBSCRIBE, optval=f"{rank}#")

    socket_sub.setsockopt_string(option=zmq.SUBSCRIBE, optval="all#")
...
class ZmqParallelization(parl_abcs.OmParallelizationBase):
    ...
    def __init__(
        ...
        *,
        ...
        self._receiver_pull.bind(
            f"tcp://{zmq_collecting.get_current_machine_ip()}:5555"
        )

        self._socket_pub = self._zmq_context.socket(zmq.PUB)
        self._socket_pub.bind(f"tcp://{zmq_collecting.get_current_machine_ip()}:5556")
